...
export default function HowItWorks() {
  return (
    <ScrollSection id="steps" className="relative z-[1] max-w-[1100px] mx-auto px-7 py-[72px]">
      ...
      <div className="grid grid-cols-3 gap-[18px]">
        {steps.map((s) => (
          <div
            key={s.step}
            className="relative p-[32px_26px] rounded-[18px] bg-white/[0.012] border border-white/[0.04]"
          >
            <span className="font-display text-[56px] font-extrabold text-brand-400/[0.04] absolute top-[10px] right-[18px] leading-none">
              {s.step}
            </span>
            <div className="relative">
              <div className="w-9 h-9 rounded-[10px] bg-brand-400/[0.06] border border-brand-400/[0.12] flex items-center justify-center mb-[18px]">
                <span className="font-display text-[13px] font-extrabold text-brand-400">{s.step}</span>
              </div>
              <h3 className="font-display text-[19px] font-bold text-gray-50 mb-2.5">{s.title}</h3>
              <p className="text-[13px] text-gray-500 leading-[1.65] mb-3.5">{s.desc}</p>
